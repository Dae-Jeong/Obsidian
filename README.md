...
- `SoT/`: 명시적으로 이관한 경력 사실·검증 근거의 로컬 정본. 다른 프로젝트는 이를 소비한다.
...
코드·원시 로그는 기존 프로젝트에 보존한다. 경력 profile/evidence는 로컬 SoT로 이관했고,
나머지 원본은 기존 owner에 연결한다. 결과를 적재할 때 출처·확인 날짜·검증 상태를 기록한다.
agent의 완료 보고와 검증 완료를 구분한다.
